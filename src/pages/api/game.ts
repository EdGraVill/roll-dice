...
export default function roomHandler(req: NextApiRequest, res: NextApiResponseWithIO) {
  const roomId = req.query?.room as string;
  const roomList = res.socket?.server?.io ? Array.from(res.socket?.server?.io._nsps.keys()) : [];
